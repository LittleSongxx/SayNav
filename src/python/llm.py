...
class LLMPlanner:
    ...
    def __init__(self, enable_room_tracking = False, model_name = 'gpt-3.5-turbo', openai_key = '', openai_base_url = ''):
        print("LLM Model Name = ", model_name)
        if openai_base_url:
            print("OpenAI Base URL = ", openai_base_url)
            self.chatllm = ChatOpenAI(temperature=0.25, max_tokens=512, model_name=model_name, openai_api_key=openai_key, openai_api_base=openai_base_url)
        else:
        ...
        if enable_room_tracking:
            if openai_base_url:
                self.chatllm_memory = ChatOpenAI(temperature=0.25, max_tokens=512, model_name=model_name, openai_api_key=openai_key, openai_api_base=openai_base_url)
            else:
